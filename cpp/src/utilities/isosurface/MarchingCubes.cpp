...
MarchingCubes::MarchingCubes(puma::Matrix<float> *workspace, std::vector<puma::Triangle<float> > *tris)
    : MarchingCubes(workspace->X(),workspace->Y(),workspace->Z() ) {
    this->workspace = workspace;
    this->tris = tris;
    this->set_resolution( workspace->X(),workspace->Y(),workspace->Z() ) ;
    this->init_all() ;
}
...
void MarchingCubes::convertToPuMA() {
    //std::cout << _ntrigs << std::endl;

    tris->resize(_ntrigs);

    for(int i=0;i< _ntrigs; i++) {
        (*tris)[i].p0.x = _vertices[_triangles[i].v3].x;
        (*tris)[i].p0.y = _vertices[_triangles[i].v3].y;
        (*tris)[i].p0.z = _vertices[_triangles[i].v3].z;
        (*tris)[i].p1.x = _vertices[_triangles[i].v2].x;
        (*tris)[i].p1.y = _vertices[_triangles[i].v2].y;
        (*tris)[i].p1.z = _vertices[_triangles[i].v2].z;
        (*tris)[i].p2.x = _vertices[_triangles[i].v1].x;
        (*tris)[i].p2.y = _vertices[_triangles[i].v1].y;
        (*tris)[i].p2.z = _vertices[_triangles[i].v1].z;
...
//_____________________________________________________________________________
// main algorithm
void MarchingCubes::run( real_mc iso )
//-----------------------------------------------------------------------------
{
...
{
    if( !_ext_data )
        _data    = new real_mc [_size_x * _size_y * _size_z] ;
...
void MarchingCubes::compute_intersection_points( real_mc iso )
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
{
    real_mc t, At=0, Bt=0, Ct=0, Dt=0, a, b ;
    char  test =  0 ;
...
real_mc MarchingCubes::get_x_grad( const int i, const int j, const int k ) const
//-----------------------------------------------------------------------------
{
    if( i > 0 )
    {
...
real_mc MarchingCubes::get_y_grad( const int i, const int j, const int k ) const
//-----------------------------------------------------------------------------
{
    if( j > 0 )
    {
...
real_mc MarchingCubes::get_z_grad( const int i, const int j, const int k ) const
//-----------------------------------------------------------------------------
{
    if( k > 0 )
    {
...
//-----------------------------------------------------------------------------
{
    ...
    real_mc u = ( _cube[0] ) / ( _cube[0] - _cube[1] ) ;
    ...
    u = (real_mc) sqrt( vert->nx * vert->nx + vert->ny * vert->ny +vert->nz * vert->nz ) ;
    if( u > 0 )
    {
        vert->nx /= u ;
        vert->ny /= u ;
...
//-----------------------------------------------------------------------------
{
    ...
    real_mc u = ( _cube[0] ) / ( _cube[0] - _cube[3] ) ;
...
//-----------------------------------------------------------------------------
{
    ...
    real_mc u = ( _cube[0] ) / ( _cube[0] - _cube[4] ) ;
